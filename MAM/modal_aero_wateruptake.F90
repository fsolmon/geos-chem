...
use physconst,        only: pi, mwh2o, rhoh2o, r_universal
...
logical, public :: use_bisection = .false.
#endif
!===============================================================================
contains
!===============================================================================
...
subroutine modal_aero_wateruptake_sub( &
   ncol, str_lev, end_lev, nmodes, rhcrystal, rhdeliques, &
   dryrad, naer, hygro, rh, dryvol, drymass, specdens_1,  &
...
! Author:  S. Ghan
!
! JS modified on 08-09-2019: follow Kai Zhang's example to make this subroutine callable by a gridcell;
!                            also move the update of dgn_wet/wetdens/qaerwat inside this subroutine
!-----------------------------------------------------------------------
   ...
   real(r8), dimension(:), intent(in)      :: rhcrystal, rhdeliques, specdens_1
   real(r8), dimension(:,:), intent(in)    :: rh
   real(r8), dimension(:,:,:), intent(in)  :: naer, dryvol, drymass, dryrad, hygro
 
   real(r8), pointer                       :: dgncur_a(:,:,:), dgncur_awet(:,:,:), qaerwat(:,:,:), wetdens(:,:,:)

   ! local variables
 
   integer :: i, k, m
   real(r8), dimension(:,:,:), allocatable :: wetrad, &             ! wet radius of aerosol (m)
...
!      real(r8), parameter :: mw      = mwh2o              ! original value: 18._r8
!!      real(r8), parameter :: pi      = 3.14159_r8        ! use the pi in the physconst
!      real(r8), parameter :: rhow    = rhoh2o / 1.e3_r8   ! original value: 1._r8 
!      real(r8), parameter :: surften = 76._r8
!      real(r8), parameter :: tair    = 273._r8
!      real(r8), parameter :: third   = 1._r8/3._r8
!      real(r8), parameter :: ugascon = r_universal*1.e4   ! 8.3e7_r8 
...
#ifdef verify_wateruptake
     real(r8)            :: q4_coeff(5)      ! coefficient for quartic equation: q45*x^4 + q44*x^3 + q43*x^2 + q42*x + q41 = 0 
#endif
...
!----------------------------------------------------------------------
   ...
   real(r8), intent(in) :: rd, c(5)
   real(r8), intent(out) :: rw

   ! Local variable
   real(r8) :: left, right, middle, f_left, f_right, f_middle
   integer  :: niter
   integer, parameter  :: niter_max = 1000
   real(r8), parameter :: toler = 10._r8 * epsilon(1._r8)      ! tolerance for bisection 
...
!----------------------------------------------------------------------
   ...
   real(r8) function quartic_func( c, x )
   implicit none
   real(r8), intent(in) :: c(5)
   real(r8), intent(in) :: x

   quartic_func = c(5)*(x**4) + c(4)*(x**3) + &
                  c(3)*(x**2) + c(2)*x + c(1)
