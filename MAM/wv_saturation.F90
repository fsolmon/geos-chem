...
subroutine qsat_water( t_in, p_in, es_out, qs_out )
  !  saturation mixing ratio w/respect to liquid water
  real(r8),intent(in)  :: t_in(:) ! temperature
  real(r8),intent(in)  :: p_in(:) ! pressure (Pa)
  real(r8),intent(out) :: es_out(:) ! saturation vapor pressure (Pa)
  real(r8),intent(out) :: qs_out(:) ! saturation vapor pressure (Pa)
  ...
  qs = epsqs*es/(p-(1.-epsqs)*es) ! saturation w/respect to liquid only
  if(qs < 0.) qs = 1.
...
real(r8) function qsat_water_fn(t,p)
  !  saturation mixing ratio w/respect to liquid water
  real(r8) t ! temperature
  ...
  qsat_water_fn = epsqs*es/(p-(1.-epsqs)*es) ! saturation w/respect to liquid only
  if(qsat_water_fn < 0.) qsat_water_fn = 1.

end function qsat_water_fn
